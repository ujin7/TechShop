...
function SubcategoryContent() {
  ...
  const query = toApiQuery({ category, subcategory, limit: 12 });
  ...
  const breadcrumbs = [
    { label: '홈', href: '/' },
    { label: categoryData?.name || category, href: `/categories/${category}` },
    ...
    <div className={styles.page}>
      <div className="app-container">
        ...
        <div className={styles.header}>
          <h1 className={styles.title}>{subcategoryData?.name || subcategory}</h1>
          {subcategoryData?.description && (
            <p className={styles.description}>{subcategoryData.description}</p>
          )}
        </div>
